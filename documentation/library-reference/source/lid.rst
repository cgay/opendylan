...
building binary for the library. Paths to search for libraries can also be
...
``library.a``:
  ...
  .. note:: This may cause a problem if you are using this to link a static
     library that hasn't been built with ``-fPIC`` into a shared library on the
     ``x86_64-linux`` platform. In general, you don't want to use this keyword
     to link a static library into a shared library since this keyword
     propagates to dependent libraries as discussed below.
