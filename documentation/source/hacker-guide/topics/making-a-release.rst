...
#. Update submodules to the latest stable version tags and document the version
   number changes in the release notes. These commands may help::

     git submodule foreach --quiet '/bin/echo -n "$name "; git describe --always --tags $sha1'
     deft list -a      # Show latest version of each package.
